...
async def _refresh_document_completeness(db, client_id: str):
    ...
    present = {
        ...
        if doc.get('status') == 'complete' and doc.get('storage_path')
    }
    required = {'ATR signed', 'Licence front', 'Licence back'}
    completeness = (
        'Complete' if required.issubset(present)
        else 'Partial' if documents
        else 'Requested'
    )
    await db.clients.update_one(
        {'id': client_id},
        {'$set': {'document_completeness': completeness, 'updated_at': datetime.now(timezone.utc)}},
    )
    return completeness
...
def _validate_document_completion(document: dict, client_documents: List[dict]):
    ...
    if document_type == 'ATR signed' and (
        not document.get('signature_present') or document.get('date_left_blank') is False
    ):
    ...
    if document_type == 'EFT form':
        ...
        if (
            document.get('customer_present') is False
            and float(document.get('amount') or 0) > 500
            and not document.get('bank_statement_received')
        ):
            raise HTTPException(
                400, 'A bank statement is required for EFT amounts over $500 when the customer is not present'
            )
...
async def _rescore_open_deals(db):
    async for client in db.clients.find(
        {'stage': {'$ne': 'Delivered'}, 'linked_offer_ids': {'$exists': True, '$ne': []}}
    ):
        offer = await db.offers.find_one(
            {'id': {'$in': client.get('linked_offer_ids', [])}, 'active': True}
        )
        if not offer:
            continue
        sale_type = client.get('sale_type', 'Retail')
        order_date = (
            client.get('order_date')
            or client.get('delivery_date')
        ...
        model_match = not models or any(
            value in model_name or model_name in value for value in models
        )
        order_match = (
            not offer.get('order_from')
            or offer.get('order_from') <= order_date <= (offer.get('order_to') or '9999-12-31')
        )
        delivery_match = (
            not offer.get('deliver_by')
            or not client.get('delivery_date')
            or client['delivery_date'] <= offer['deliver_by']
        )
        status_value, reason = 'Eligible', None
        if sale_type.lower() in exclusions:
            status_value, reason = (
                'Ineligible',
                f'{sale_type} sale excluded from {offer.get("name", "offer")}',
            )
...
def _validate_sale_type(sale_type: str, fleet_company: Optional[str]):
    if sale_type in ('Novated lease', 'Fleet') and not fleet_company:
        raise HTTPException(
            400, 'Fleet / FMO company is required for Fleet and Novated lease deals'
        )
...
def _has_signed_handover(client: dict) -> bool:
    return client.get('handover_checklist_status') == 'Signed copy on file' and any(
        document.get('document_type') == 'Handover checklist'
...
# ---------------------------------------------------------------------------
# Clients — collection-level routes  (no path parameter)
# ---------------------------------------------------------------------------
...
async def create_client(payload: ClientBase, user: User = Depends(get_current_user)):
    ...
    if payload.activation_status in ('Ready', 'Submitted to BYD', 'Active') and not _has_signed_handover(
        payload.model_dump()
    ):
        raise HTTPException(
            400, 'Activation cannot be set to Ready while handover checklist is not on file'
        )
...
# ---------------------------------------------------------------------------
# Clients — static sub-collection routes  (must precede /{client_id})
# ---------------------------------------------------------------------------
    ...
    async for client in db.clients.find({'stage': {'$ne': 'Delivered'}}).limit(500):
        ...
        if valid_until:
            try:
                ...
                if days <= 10:
                    alerts.append({
                        'id': f'trade-{client_id}',
                        'type': 'trade-in',
                        'severity': 'danger' if days <= 0 else 'warning',
                        'client_id': client_id,
                        'message': (
                            f'{name}: trade-in valuation expires in {max(days, 0)} day(s)'
                            if days >= 0
                            else f'{name}: trade-in valuation expired'
                        ),
                    })
            except ValueError:
                pass
        if client.get('document_completeness') in ('Requested', 'Partial'):
            alerts.append({
                ...
                'severity': 'warning',
                'client_id': client_id,
                'message': f'{name}: registration documents are {str(client.get("document_completeness")).lower()}',
            })
        for offer_id in client.get('linked_offer_ids', []):
            ...
            for field, label in (('order_to', 'order window'), ('deliver_by', 'delivery deadline')):
                ...
                if days in (14, 7, 3, 1) or days <= 0:
                    alerts.append({
                        'id': f'offer-{client_id}-{offer_id}-{field}',
                        'type': 'offer',
                        'severity': 'danger' if days <= 0 else 'warning',
                        'client_id': client_id,
                        'message': (
                            f'{name}: {offer.get("name", "offer")} {label} is '
                            f'{"expired" if days <= 0 else f"closing in {days} day(s)"}'
                        ),
                    })
    return alerts
...
async def receive_inbound_document(
    ...
    doc = ClientDocument(
        ...
        source='email',
        uploaded_by=user.id,
        storage_path=f'{client["id"]}/{safe_name}',
    ...
    await db.audit.insert_one({
        ...
        'action': 'document.receive.classify.attach',
        'entity': 'client',
        'entity_id': client['id'],
        'meta': {
            'document_id': doc.id,
...
async def inbound_queue(user: User = Depends(get_current_user), limit: int = 100):
    ...
    sla_hours = 4
    items = []
    async for event in db.audit.find(
        {'action': 'document.receive.unmatched'}
...
async def resolve_inbound_item(
    event_id: str, payload: dict, user: User = Depends(get_current_user)
):
    ...
    await db.audit.update_one(
        {'id': event_id},
        {
            '$set': {
                'action': 'document.receive.unmatched.resolved',
                'meta.resolved_by': user.id,
                'meta.resolved_client_id': client_id,
...
# ---------------------------------------------------------------------------
# Offers — static sub-collection routes  (must precede /{client_id})
# ---------------------------------------------------------------------------
...
async def fetch_offer_website_snapshot(user: User = Depends(get_current_user)):
    ...
    snapshot = {
        ...
        'content_preview': re.sub(r'\s+', ' ', content)[:2000],
        'changed': not previous or previous.get('sha256') != digest,
        'fetched_by': user.id,
    }
    ...
    await db.audit.insert_one({
        ...
        'actor_id': user.id,
        'actor_email': user.email,
        'action': 'offers.website_snapshot',
        'entity': 'offer_catalogue',
        'entity_id': None,
        'meta': {'url': url, 'sha256': digest, 'changed': snapshot['changed']},
        'created_at': datetime.now(timezone.utc),
    })
    return snapshot
...
async def refresh_offers(user: User = Depends(get_current_user)):
    ...
    await db.audit.insert_one({
        ...
        'actor_id': user.id,
        'actor_email': user.email,
        'action': 'offers.refresh',
        'entity': 'offer_catalogue',
        'entity_id': None,
        'meta': {'active_offers': result.modified_count, 'refreshed_at': refreshed_at},
...
async def update_offer(offer_id: str, payload: OfferUpdate, user: User = Depends(require_admin)):
    ...
    record = await db.offers.find_one_and_update(
        {'id': offer_id}, {'$set': update}, return_document=True
    )
    if not record:
...
# ---------------------------------------------------------------------------
# Upload links — static sub-collection routes  (must precede /{client_id})
# ---------------------------------------------------------------------------
...
async def upload_link_info(token: str):
    ...
    return {
        'client_name': client.get('name') if client else 'Customer',
        'vehicle': client.get('vehicle') if client else None,
...
async def upload_from_customer_link(
    ...
    doc = ClientDocument(
        ...
        file_name=file.filename,
        source='email',
        storage_path=f'{link["client_id"]}/{safe_name}',
        content_type=file.content_type,
        size_bytes=len(data),
        notes='Received through customer upload link',
    )
    ...
    await db.audit.insert_one({
        ...
        'actor_id': None,
        'actor_email': None,
        'action': 'document.receive.portal_attach',
        'entity': 'client',
...
# ---------------------------------------------------------------------------
# Clients — single-resource routes  /{client_id}
# ---------------------------------------------------------------------------
...
async def update_client(
    ...
    if not existing:
        raise HTTPException(404, 'Client not found')
    _validate_sale_type(
        update.get('sale_type', existing.get('sale_type', 'Retail')),
        update.get('fleet_company', existing.get('fleet_company')),
    )
    if 'linked_offer_ids' in update:
        await _validate_offer_selection(db, update['linked_offer_ids'])
    if (
        update.get('your_way_selection')
        and existing.get('your_way_selection')
        and update['your_way_selection'] != existing['your_way_selection']
    ):
        raise HTTPException(400, 'Your Way selection is locked once confirmed')
    merged_handover = {
        **existing,
        **({'handover_checklist_status': update.get('handover_checklist_status')}
           if 'handover_checklist_status' in update else {}),
    }
    if update.get('activation_ready') is True and not _has_signed_handover(merged_handover):
        raise HTTPException(
            400, 'Activation cannot be set to Ready while handover checklist is not on file'
        )
    if update.get('activation_status') in ('Ready', 'Submitted to BYD', 'Active') and not _has_signed_handover(
        merged_handover
    ):
        raise HTTPException(
            400, 'Activation cannot be set to Ready while handover checklist is not on file'
    ...
    if (
        update.get('handover_checklist_status') != 'Signed copy on file'
        and existing.get('handover_checklist_status') == 'Signed copy on file'
        and existing.get('activation_ready')
    ):
    ...
    if (
        update.get('trade_in_status') == 'Settled'
        and trade_in_valid_until
        and trade_in_valid_until < datetime.now(timezone.utc).date().isoformat()
    ):
...
# ---------------------------------------------------------------------------
# Clients — action routes  /{client_id}/...
# ---------------------------------------------------------------------------
...
async def generate_client_pack(client_id: str, user: User = Depends(get_current_user)):
    ...
    doc = ClientDocument(
        ...
        notes='Generated working pack. Replace overlays with approved Harmony/VicRoads templates when supplied.',
    )
    await db.clients.update_one(
        {'id': client_id},
        {'$push': {'documents': doc.model_dump(mode='json')}, '$set': {'updated_at': generated_at}},
    )
    await _refresh_document_completeness(db, client_id)
    await db.audit.insert_one({
        'id': uuid.uuid4().hex,
        'actor_id': user.id,
        'actor_email': user.email,
        'action': 'document.pack.generate',
        'entity': 'client',
        'entity_id': client_id,
        'meta': {
            'document_id': doc.id,
            'file_name': file_name,
            'merge_fields': {field: client.get(field) for field in required},
    ...
    return {
        'success': True,
        'document': doc,
        'email_subject': f"Your {client['vehicle']} order {client['po_number']} - documents required for registration",
    }
...
async def send_client_pack(client_id: str, user: User = Depends(get_current_user)):
    ...
    pack = next(
        (
            doc
            for doc in reversed(client.get('documents', []))
            if str(doc.get('file_name', '')).startswith('generated-pack-')
        ),
        None,
    )
...
'''
    try:
        await send_email(
            ...
            attachments=[(
                pack['file_name'],
                await get_bytes(pack['storage_path']),
                pack.get('content_type') or 'application/pdf',
            )],
        )
    except Exception as exc:
    ...
    await db.clients.update_one(
        {'id': client_id, 'documents.id': pack['id']},
        {'$set': {'documents.$.sent_at': sent_at, 'updated_at': sent_at}},
    )
    await db.audit.insert_one({
        'id': uuid.uuid4().hex,
        'actor_id': user.id,
        'actor_email': user.email,
        'action': 'document.pack.send',
        'entity': 'client',
        'entity_id': client_id,
        'meta': {'document_id': pack['id'], 'recipient': client['email'], 'subject': subject},
        'created_at': sent_at,
    })
...
async def create_client_upload_link(client_id: str, user: User = Depends(get_current_user)):
    ...
    await db.upload_links.insert_one({
        'id': uuid.uuid4().hex,
        'token': token,
        'client_id': client_id,
...
async def generate_offer_claim_pack(client_id: str, user: User = Depends(get_current_user)):
    ...
    claim_types = offer.get('claim_doc_templates') or (
        [offer.get('claim_doc_template')] if offer.get('claim_doc_template') else []
    )
    ...
    doc = ClientDocument(
        ...
        content_type='application/pdf',
        size_bytes=len(pack_data),
        notes=f'Claim artefacts required: {", ".join(claim_types) if claim_types else "offer rule"}',
    )
    await db.clients.update_one(
        {'id': client_id},
        {'$push': {'documents': doc.model_dump(mode='json')}, '$set': {'updated_at': generated_at}},
    ...
    await db.audit.insert_one({
        ...
        'actor_id': user.id,
        'actor_email': user.email,
        'action': 'offer.claim.generate',
        'entity': 'client',
        'entity_id': client_id,
        'meta': {'offer_id': offer['id'], 'document_id': doc.id, 'claim_types': claim_types},
        'created_at': generated_at,
    })
    return doc
...
async def match_client_offers(client_id: str, user: User = Depends(get_current_user)):
    ...
    async for offer in db.offers.find({'active': True}).sort('name', 1):
        ...
        exclusions = [str(value).lower() for value in offer.get('sale_type_exclusions', [])]
        model_match = not models or any(
            model in model_name or model_name in model for model in models
        )
        ...
        reason = (
            f'{sale_type} sale excluded' if excluded
            else 'Model or order window does not match' if not model_match or not in_window
            else 'Delivery deadline is after offer deadline' if not delivery_ok
            else None
        )
...
async def update_client_offer_status(
    client_id: str, payload: dict, user: User = Depends(get_current_user)
):
    db = get_db()
    update = {
        'offer_status': payload.get('offer_status', 'Eligible'),
        'offer_reason': payload.get('offer_reason'),
        'updated_at': datetime.now(timezone.utc),
    }
...
async def update_trade_in(
    ...
    if 'trade_in_status' in payload:
        if payload['trade_in_status'] not in (
            'Pending', 'Quoted', 'Accepted', 'Vehicle received', 'Settled',
            'Valid', 'Expiring soon', 'Expiring', 'Expired', 'Cancelled',
        ):
        ...
        if (
            payload['trade_in_status'] == 'Settled'
            and valid_until
            and valid_until < datetime.now(timezone.utc).date().isoformat()
            and not payload.get('trade_in_manager_reason')
        ):
            raise HTTPException(400, 'A manager reason is required to settle an expired trade-in')
        if (
            payload['trade_in_status'] == 'Settled'
    ...
    await db.audit.insert_one({
        ...
        'actor_id': user.id,
        'actor_email': user.email,
        'action': 'trade_in.status_change',
        'entity': 'client',
        'entity_id': client_id,
        'meta': {k: v for k, v in update.items() if k != 'updated_at'},
        'created_at': datetime.now(timezone.utc),
    })
    return {'success': True}
...
async def update_activation_gate(
    client_id: str, payload: dict, user: User = Depends(get_current_user)
):
    db = get_db()
    client = await db.clients.find_one({'id': client_id})
    ...
    if activating and not handover and (
        user.role not in ('admin', 'super_admin') or not override_reason
    ):
        raise HTTPException(
            400, 'Activation cannot be set to Ready while handover checklist is not on file'
        )
    ...
    await db.audit.insert_one({
        ...
        'actor_id': user.id,
        'actor_email': user.email,
        'action': 'activation.status_change',
        'entity': 'client',
        'entity_id': client_id,
        'meta': update,
        'created_at': datetime.now(timezone.utc),
...
async def add_comment(
    client_id: str, payload: CommentCreate, user: User = Depends(get_current_user)
):
    db = get_db()
...
async def add_accessory(
    client_id: str, payload: AccessoryCreate, user: User = Depends(get_current_user)
):
    db = get_db()
    ...
    res = await db.clients.update_one(
        {'id': client_id},
        {
            '$push': {'accessories': acc.model_dump(mode='json')},
            '$set': {'updated_at': datetime.now(timezone.utc)},
        },
    )
    if res.matched_count == 0:
...
async def list_client_documents(client_id: str, user: User = Depends(get_current_user)):
    ...
    if not client:
        raise HTTPException(404, 'Client not found')
    return [ClientDocument(**d) for d in client.get('documents', [])]


@router.post('/{client_id}/documents', response_model=ClientDocument)
async def add_client_document(
    client_id: str, payload: ClientDocumentUpdate, user: User = Depends(get_current_user)
):
    db = get_db()
    client = await db.clients.find_one({'id': client_id})
    if not client:
    ...
    if payload.status == 'complete':
        raise HTTPException(
            400, 'Complete documents must be uploaded through the document upload endpoint'
        )
    ...
    await db.audit.insert_one({
        ...
        'entity': 'client',
        'entity_id': client_id,
        'meta': {'document_id': doc.id, 'document_type': doc.document_type, 'file_name': doc.file_name},
        'created_at': datetime.now(timezone.utc),
    })
    return doc
...
async def upload_client_document(
    ...
    await db.audit.insert_one({
        ...
        'actor_id': user.id,
        'actor_email': user.email,
        'action': 'document.receive.attach',
        'entity': 'client',
        'entity_id': client_id,
        'meta': {
            'document_id': doc.id,
...
async def download_client_document(
    client_id: str, document_id: str, user: User = Depends(get_current_user)
):
    db = get_db()
    client = await db.clients.find_one({'id': client_id})
    if not client:
        raise HTTPException(404, 'Client not found')
    document = next(
        (item for item in client.get('documents', []) if item.get('id') == document_id), None
    )
    ...
    if not await exists(document['storage_path']):
        raise HTTPException(404, 'Document file not found')
    return Response(
        content=await get_bytes(document['storage_path']),
        media_type=document.get('content_type') or 'application/octet-stream',
        headers={'Content-Disposition': f'attachment; filename="{document.get("file_name") or "document"}"'},
    )
...
async def update_client_document(
    ...
    existing_document = await db.clients.find_one(
        {'id': client_id, 'documents.id': document_id}, {'documents.$': 1}
    )
    document = (existing_document or {}).get('documents', [{}])[0]
    merged_document = {
        **document,
        **{key.split('.', 2)[-1]: value for key, value in update.items()},
    }
    _validate_document_completion(
        merged_document, (existing_document or {}).get('documents', [])
    )
    res = await db.clients.find_one_and_update(
    ...
    document = next(
        item for item in res.get('documents', []) if item.get('id') == document_id
    )
    await _refresh_document_completeness(db, client_id)
    await db.audit.insert_one({
        'id': uuid.uuid4().hex,
        ...
        'entity': 'client',
        'entity_id': client_id,
        'meta': {'document_id': document_id, 'changes': payload.model_dump(exclude_unset=True)},
        'created_at': datetime.now(timezone.utc),
    })
