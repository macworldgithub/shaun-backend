...
async def create_client(payload: ClientBase):
    db = get_db()
    ...
    if payload.vy_order_id:
        ...
        if existing:
            raise HTTPException(
                409,
                'Client with this VY order ID already exists'
            )
...
async def update_client(
    ...
    # Auto stamp arrived_at
    if 'arrived' in update and update['arrived'] is True:
        update['arrived_at'] = datetime.now(timezone.utc)

    # Auto stamp last_contacted_at
    if (
        'contact_status' in update and
        update['contact_status'] in (
            'Contacted',
            'Booked',
            'Awaiting Reply'
        )
...
async def add_accessory(
    ...
    res = await db.clients.update_one(
        ...
        {
            '$push': {
                'accessories': acc.model_dump(mode='json')
            },
            '$set': {
                'updated_at': datetime.now(timezone.utc)
            }
...
async def bulk_import(items: List[ClientBase]):
    db = get_db()
    ...
    for item in items:
        ...
        data['imported_from'] = (
            data.get('imported_from') or 'paste'
        )
